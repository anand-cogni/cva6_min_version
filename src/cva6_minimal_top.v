...
module cva6_minimal_top (
    ...
    // ========================================================================
    // Memory-to-AXI Bridge Instance (NOT USED - HBM3 uses simple memory interface)
    // ========================================================================
    // Note: The AXI bridge is instantiated but not connected to HBM3
    // Could be used for other peripherals that need AXI in the future
    /* DISABLED FOR NOW - HBM3 has simple memory interface, no AXI needed
    mem_to_axi_bridge u_mem_to_axi_bridge (
        .clk            (clk),
        .resetn         (aresetn),
        
        // Memory Interface (from simple_riscv_core)
        .mem_req        (1'b0),               // Not connected
        .mem_we         (1'b0),
        .mem_be         (4'h0),
        .mem_addr       (32'h0),
        .mem_wdata      (32'h0),
        .mem_rdata      (),                   // Unconnected
        .mem_valid      (),                   // Unconnected
        .mem_ready      (),                   // Unconnected
    ...
    */
    ...
    // ========================================================================
    // HBM3 Controller Instance - DISABLED
    // ========================================================================
    /* DISABLED - HBM3 Controller removed from build
    hbm3_controller #(
        .DATA_WIDTH (32),
        .ADDR_WIDTH (32),
        .MEM_SIZE   (32'h10000000)  // 256MB
    ) u_hbm3_controller (
        // Clock and Reset
        .clk        (clk),
        .rst_n      (aresetn),
        
        // Configuration Register Interface (via APB)
        .cfg_req    (apb_psel && apb_penable),
        .cfg_we     (apb_pwrite),
        .cfg_addr   (apb_paddr[6:2]),
        .cfg_wdata  (apb_pwdata),
        .cfg_be     (4'hF),
        .cfg_rdata  (apb_prdata),
        .cfg_ready  (apb_pready),
        
        // Memory Access Interface (from mem_to_axi_bridge via simple conversion)
    ...
    */
    ...
    // Tie off HBM3 signals when controller is disabled
    assign hbm3_rdata = 32'hDEADBEEF;  // Return dummy data
    assign hbm3_valid = 1'b0;           // Never valid
    assign hbm3_ready = 1'b0;           // Never ready
    assign apb_prdata = 32'h0;          // APB read data
    assign apb_pready = 1'b1;           // APB always ready
    assign apb_pslverr = 1'b0;          // No APB errors
    
    // Tie off HBM3 physical interface (for simulation - no actual DRAM)
    assign phy_to_dram_hbm_ck_t = 1'b0;
    assign phy_to_dram_hbm_ck_c = 1'b1;
    assign phy_to_dram_hbm_cke = 1'b0;
    assign phy_to_dram_hbm_cs_n = 1'b1;
    assign phy_to_dram_hbm_ca = 6'h0;
    // Note: phy_to_dram_io_hbm_dq and phy_to_dram_io_hbm_ecc are inout - leave unconnected
    assign phy_to_dram_hbm_dm = 16'h0;
